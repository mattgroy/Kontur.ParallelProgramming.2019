...
//				Console.WriteLine("Main thread finishing");
        ...
        public static AutoResetEvent ApmTest()
        {
            ...
            var fStream = new FileStream("file1.txt", FileMode.Create, FileAccess.ReadWrite, FileShare.None,
                WRITE_BLOCK_SIZE, FileOptions.WriteThrough);
            ...
            fStream.BeginWrite(buf, 0, buf.Length,
                asyncResult =>
                {
                    ...
                    evnt.Set();
                    fStream.Dispose();
                    Console.WriteLine("Finished disk I/O! in {0} ms (thread {1})", sw.ElapsedMilliseconds,
                        Thread.CurrentThread.ManagedThreadId);
                }, fStream);
            Console.WriteLine("Started disk I/O! in {0} ms total (thread {1})", sw.ElapsedMilliseconds,
                Thread.CurrentThread.ManagedThreadId);
        ...
        public static Task FileContinueWith()
        {
            ...
            var fStream = new FileStream("file2.txt", FileMode.Create, FileAccess.ReadWrite, FileShare.None,
                WRITE_BLOCK_SIZE, FileOptions.WriteThrough);

            var sw = Stopwatch.StartNew();

            var copyToAsyncTask = fStream.WriteAsync(buf, 0, buf.Length);
            Console.WriteLine("Started disk I/O! in {0} ms (thread {1})", sw.ElapsedMilliseconds,
                Thread.CurrentThread.ManagedThreadId);
            ...
            var resultTask = copyToAsyncTask.ContinueWith(writeTask =>
            {
                ...
                if (writeTask.IsFaulted)
                    throw new Exception("", writeTask.Exception);
                Console.WriteLine("Finished disk I/O! in {0} ms total (thread {1})", sw.ElapsedMilliseconds,
                    Thread.CurrentThread.ManagedThreadId);
            });
        ...
        public static async Task FileAsync()
        {
            ...
            using (var fStream = new FileStream("file3.txt", FileMode.Create, FileAccess.ReadWrite, FileShare.None,
                WRITE_BLOCK_SIZE, FileOptions.WriteThrough))
            {
                var sw = Stopwatch.StartNew();
...
//			Console.WriteLine(Thread.CurrentThread.ManagedThreadId);
        ...
        private static async Task<string> HashFileAsync(string filename)
        {
            using (var ms = new MemoryStream())
            {
                ...
                {
                    Console.WriteLine("Starting async read of file {0} [thread {1}]", filename,
                        Thread.CurrentThread.ManagedThreadId);
                    await ifs.CopyToAsync(ms);
                }

                Console.WriteLine("Read file {0} in {1} ms  [thread {2}]", filename, sw.ElapsedMilliseconds,
                    Thread.CurrentThread.ManagedThreadId);
                sw.Restart();
                var hash = MD5.Create().ComputeHash(ms.GetBuffer(), 0, (int) ms.Position);
                Console.WriteLine("Computed hash of file {0} in {1} ms [thread {2}]", filename, sw.ElapsedMilliseconds,
                    Thread.CurrentThread.ManagedThreadId);
        ...
        private static Task<double> CosCalcAsync()
        {
            ...
            {
                double cur = 1;
                for (var i = 0; i < 1 * 1000 * 1000; i++) cur = Math.Cos(cur);
